...
class GeneticAlgorithmTSP:
    ...
    def randomizeCities(self, graph_nodes):
        ...
        """
        # Remove Curitiba and Salvador from the nodes list to prevent them from being shuffled
        nodes = [node for node in graph_nodes if node != self.graph.start_city and node != self.graph.end_city]

        # Generate routes where Curitiba is the start city and Salvador is the end city
        return [
            self.graph.start_city + ''.join(rd.sample(nodes, len(nodes))) + self.graph.end_city
