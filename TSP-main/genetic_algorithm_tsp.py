...
class GeneticAlgorithmTSP:
    ...
    def create_next_generation(self, graph, population, number_of_fits_to_carryover):
        ...
        new_population += [self.mutate(self.crossover(*self.select_parents(graph, population))) for _ in range(self.population_size - number_of_fits_to_carryover)]
    ...
    def randomizeCities(self, graph_nodes, max_cities=5):
        """
        Randomly generate routes for the initial population, with a random number of cities to visit, avoiding duplicates.

        Parameters:
        - graph_nodes: List of nodes in the TSP graph.
        - max_cities: Maximum number of cities to include in the route (up to max_cities cities).

        Returns:
        - List of random routes with cities visited only once.
        """
        nodes = [node for node in graph_nodes if node != self.graph.start_city and node != self.graph.end_city]

        # Choose a random number of cities to visit, up to max_cities (which is 5 in this case)
        num_cities = rd.randint(2, max_cities)  # At least 2 cities (start and end city excluded)

        # Select a subset of cities to be visited, ensuring no duplicates
        selected_cities = rd.sample(nodes, num_cities)

        # Generate routes where the start city and end city are fixed, but some cities are omitted
        return [
            self.graph.start_city + ''.join(rd.sample(selected_cities, len(selected_cities))) + self.graph.end_city
            for _ in range(self.population_size)
        ]

    def computeFitness(self, graph, population):
        """
        Compute the fitness (cost) for each route in the population, even if not all cities are visited.
        ...
        for path in population:
            # Extract the cities in the current path, excluding the start and end cities
            cities_in_path = [city for city in path[1:-1] if city != self.graph.start_city and city != self.graph.end_city]
            
            # Calculate the cost of the path with the cities included
            path_cost = graph.getPathCost([self.graph.start_city] + cities_in_path + [self.graph.end_city])
            fitness_values.append(path_cost)
    ...
    def crossover(self, parent1, parent2):
        """
        Perform crossover between two parents to generate an offspring, allowing missing cities.
        ...
        offspring = [self.graph.start_city] + offspring + [self.graph.end_city]
